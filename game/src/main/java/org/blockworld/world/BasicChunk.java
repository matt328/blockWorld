...
public class BasicChunk implements Chunk {
    ...
    public BasicChunk(Vector3f dimensions, Vector3f position) {
        ...
        boundingBox = new BoundingBox(new Vector3f(centerX, centerY, centerZ), dimensions.x / 2, dimensions.y / 2,
                dimensions.z / 2);
     ...
     *                                  if the local position is found to be outside
     *                                  this
     *                                  {@link Chunk}'s range
     */
    private void checkArrayAccess(Vector3f globalPosition, Vector3f localPosition) {
        Preconditions.checkArgument(localPosition.x >= 0,
                "Position X Value (%s) produces translated value less than zero", globalPosition.x);
        Preconditions.checkArgument(localPosition.y >= 0,
                "Position Y Value (%s) produces translated value less than zero", globalPosition.y);
        Preconditions.checkArgument(localPosition.z >= 0,
                "Position Z Value (%s) produces translated value less than zero", globalPosition.z);

        Preconditions.checkArgument(localPosition.x < dimensions.x,
                "Position X Value (%s) would cause access outside of bounds (Max X: %s)", localPosition.x,
                dimensions.x - 1);
        Preconditions.checkArgument(localPosition.y < dimensions.y,
                "Position Y Value (%s) would cause access outside of bounds (Max Y: %s)", localPosition.y,
                dimensions.y - 1);
        Preconditions.checkArgument(localPosition.z < dimensions.z,
                "Position Z Value (%s) would cause access outside of bounds (Max Z: %s)", localPosition.z,
                dimensions.z - 1);
    }
