...
public class MeshChunkNode extends AbstractChunkNode {
    ...
    private static final ArrayList<Vector3f> NORMALS_DOWNFACE = Lists.newArrayList(NORMAL_DOWN, NORMAL_DOWN,
            NORMAL_DOWN, NORMAL_DOWN);
    private static final ArrayList<Vector3f> NORMALS_UPFACE = Lists.newArrayList(NORMAL_UP, NORMAL_UP, NORMAL_UP,
            NORMAL_UP);
    private static final ArrayList<Vector3f> NORMALS_LEFTFACE = Lists.newArrayList(NORMAL_LEFT, NORMAL_LEFT,
            NORMAL_LEFT, NORMAL_LEFT);
    private static final ArrayList<Vector3f> NORMALS_RIGHTFACE = Lists.newArrayList(NORMAL_RIGHT, NORMAL_RIGHT,
            NORMAL_RIGHT, NORMAL_RIGHT);
    private static final ArrayList<Vector3f> NORMALS_FRONTFACE = Lists.newArrayList(NORMAL_FRONT, NORMAL_FRONT,
            NORMAL_FRONT, NORMAL_FRONT);
    private static final ArrayList<Vector3f> NORMALS_BACKFACE = Lists.newArrayList(NORMAL_BACK, NORMAL_BACK,
    ...
    public MeshChunkNode(final Chunk terrainChunk, final TextureAtlas atlas, final BlockworldBlockLoader loader,
        ...
        super(terrainChunk, theFunction);
        this.loader = loader;
        this.atlas = atlas;
    ...
    protected List<Geometry> createGeometries() {
        ...
        for (int x = sx; x < ex; x++) {
            for (int y = sy; y < ey; y++) {
                for (int z = sz; z < ez; z++) {
                    var noiseValue = this.theFunction.get(x, y, z);
                    final Vector3f blockPosition = new Vector3f(x, y, z);
                    final int blockType = noiseValue > 0 ? 1 : 0;
                    if (blockType != 0) {
        ...
        while (it.hasNext()) {
            ...
            chunkMesh.setBuffer(Type.Position, 3,
                    BufferUtils.createFloatBuffer(vertices.get(blockType).toArray(new Vector3f[vertices.size()])));
            chunkMesh.setBuffer(Type.Normal, 3,
                    BufferUtils.createFloatBuffer(normals.get(blockType).toArray(new Vector3f[normals.size()])));
            chunkMesh.setBuffer(Type.TexCoord, 2,
                    BufferUtils.createFloatBuffer(texCoord.get(blockType).toArray(new Vector2f[texCoord.size()])));
            chunkMesh.setBuffer(Type.Index, 1, BufferUtils
                    .createIntBuffer(ArrayUtils.toPrimitive(indexes.get(blockType).toArray(new Integer[0]))));
            chunkMesh.setBuffer(Type.TexCoord2, 2,
                    BufferUtils.createFloatBuffer(lightCoord.get(blockType).toArray(new Vector2f[lightCoord.size()])));
            chunkMesh.updateBound();
            chunkMesh.setStatic();
            if (chunkMesh.getVertexCount() != 0) {
                final Material material = atlas.getMaterial(blockType);
                final Geometry geometry = GeometryBuilder.geometry("b-" + blockType).mesh(chunkMesh).material(material)
                        .get();
    ...
    private EnumSet<Face> checkFaces(final Vector3f blockPosition, final float dimension) {
        ...
        final int currentBlock = theFunction.get(x, y, z) > 0 ? 1 : 0;
    ...
    private boolean needFace(final int currentBlock, final float x, final float y, final float z,
            final Face faceToCheck) {
        return needFace(currentBlock, theFunction.get(x, y, z) > 0 ? 1 : 0);
    }
    ...
    private boolean createFaces(final Vector3f position, final float dimension, final EnumSet<Face> faces,
            final int bloxelType) {
        if (faces.isEmpty()) {
            return false;
        }
