...
public final class Noise2 {
	...
	private final double g2[][] = new double[B + B + 2][2];
	private final double g1[] = new double[B + B + 2];
	// private static int start = 1;
	private final double[][] points = new double[32][3];

	public Noise2(final int newSeed) {
		seed = newSeed;
		int i, j, k;
		...
		final double r2 = Math.sqrt(1. / 2);
		final double s = Math.sqrt(2 + r2 + r2);
	...
	private double s_curve(final double t) {
		return t * t * (3 - t - t);
	}
	...
	private double[] G(final int i) {
		return points[i % 32];
	}

	private void normalize2(final double v[]) {
		double s;
