 ...
 * 
 */
public class TerasologyTerrainLoader implements BlockLoader<Block> {
	...
	public TerasologyTerrainLoader(final String seed) {
	...
	@Override
	public void fill(BlockVolume<Block> volume) {
			...
			for (int z = 0; z < dimZ; z++) {
				...
				for (int y = dimY; y >= 0; y--) {
					...
					if (c.y <= 0) {
						...
						if (caveNoise > 0.24f) {
							volume.setBlock(c.x, c.y, c.z, new Block(c, elementSize, 4));
						}
						continue;
					}
					if (c.y < WATER_LEVEL_Y && c.y > 0) {
						// Ocean
						volume.setBlock(c.x, c.y, c.z, new Block(c, elementSize, 6));
					}
					...
					if ((dens >= 0.01f && dens < 0.012f)) {
						...
						if (firstBlockHeight == -1) {
							volume.setBlock(c.x, c.y, c.z, new Block(c, elementSize, getBlockTailpiece(getBlockTypeForPosition(c.y, 1.0f), c.y)));
							// Generate lakes
							final float lakeIntensity = calcLakeIntensity(x + offsetX, z + offsetZ);
							// System.out.println("ld:" + lakeIntensity);
							if (lakeIntensity < 0.1) {
								volume.setBlock(c.x, c.y, c.z, new Block(c, elementSize, 7));
							}
							firstBlockHeight = c.y;
						} else {
							volume.setBlock(c.x, c.y, c.z, new Block(c, elementSize, getBlockTypeForPosition(c.y, 1.0f - ((firstBlockHeight - c.y) / SAMPLE_RATE_3D_VERT))));
						}
					} else if (dens >= 0.012f) {
						volume.setBlock(c.x, c.y, c.z, new Block(c, elementSize, getBlockTailpiece(getBlockTypeForPosition(c.y, 0.2f), c.y)));
						if (firstBlockHeight == -1) {
							firstBlockHeight = c.y;
						}
					}
