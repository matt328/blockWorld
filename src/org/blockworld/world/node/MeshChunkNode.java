...
public class MeshChunkNode extends AbstractChunkNode {
	...
	protected List<Geometry> createGeometries() {
		int c = 0;
		TIntSet usedBlockTypes = new TIntHashSet();
		Vector3f chunkCenter = terrainChunk.getBoundingBox().getCenter();
		int sx = (int) (chunkCenter.x - terrainChunk.getBoundingBox().getXExtent());
		int sy = (int) (chunkCenter.y - terrainChunk.getBoundingBox().getYExtent());
		int sz = (int) (chunkCenter.z - terrainChunk.getBoundingBox().getZExtent());
