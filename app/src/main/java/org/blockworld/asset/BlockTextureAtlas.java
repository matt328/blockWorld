/**
 * BlockTextureAtlas Author: Matt Teeter Apr 22, 2012
 */
...
public class BlockTextureAtlas implements TextureAtlas {
    ...
    */
   private static final float ATLAS_SIZE = 512f;

   /**
    * Size of each image in the texture.
    */
   private static final float IMAGE_SIZE = 32f;
   ...
   public BlockTextureAtlas(final AssetManager theAssetManager) {
      ...
      try {
         final URL url = ClassLoader.getSystemClassLoader().getResource("Textures/blockAtlas.properties");
         if (url == null) {
      ...
      // theAssetManager.loadTexture("Textures/Bloxels/minecraft_normal.png");
      ...
      bloxelMaterial.setBoolean("VertexLighting", true); // need to avoid shader
      // error!
      // "missing vNormal" ?!
   ...
   @Override
   public final Set<Integer> getBlockTypes() {
      return types;
   }

   @Override
   public final Material getMaterial(final Integer blockType) {
   ...
   @Override
   public final List<Vector2f> getTextureCoordinates(final Integer bloxelType, final int face) {
      return uvMapTexture(uvmap.get(new UVMapKey(bloxelType, face)));
   }

   @Override
   public final boolean isTransparent(final Integer bloxelType) {
   ...
   public final void setWireframe(final boolean newWireframe) {
   ...
   private List<Vector2f> uvMapTexture(final Vector2f coord) {
      ...
      // coord.x: 0..15
      // coord.y: 0..15
      final float s = IMAGE_SIZE / ATLAS_SIZE;
   ...
   @Override
   public final Material getRedMaterial() {
      return redMaterial;
   }

   @Override
   public final Material getBlueMaterial() {
      return blueMaterial;
   }
}
